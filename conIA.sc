...
// 1
val nuevalista = estudiantes.filter(_.notas.size >= 3 )
// 2
val notasFinales = nuevalista.map(e => e.notas.max - e.notas.min)
//3
case class NotaEstudiante(nombre: String, diferencia: Double)

val resultado = estudiantes.map(e =>
  nuevalista.map(e => NotaEstudiante(e.nombre, e.notas.max - e.notas.min))
)
